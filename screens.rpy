...
screen input(prompt):
    ...
    window:
        # Эта часть отвечает за положение всего окна на экране
        xalign 0.5
        yalign 0.45  # Подбери это число, чтобы текст попал ровно в центр твоей рамки
        top_padding 50
        vbox:
            xalign 0.5 # Центрируем содержимое по горизонтали

            text prompt:
                style "input_prompt"
                xalign 0.5      # Центрируем текст вопроса
                text_align 0.5  # Выравниваем текст вопроса по центру
                outlines [ (2, "#000000", 0, 0) ]

            input id "input":
                xalign 0.5      # Центрируем строку ввода
                text_align 0.5  # Чтобы печаталось из центра в стороны
                outlines [ (2, "#3e5f8a", 0, 0) ]
                color "#00fbff"
